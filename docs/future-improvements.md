...
This document collects broad selection principles and deferred improvement ideas
without presenting them as implemented capabilities. The current actionable list
is [the backlog](BACKLOG.md). An item belongs in current operator documentation
only after its implementation and evidence are accepted.
...
abstractions, compatibility surfaces, and autonomous loops require evidence that
the existing design is insufficient.

## Release-path improvements

These improvements are part of the active path toward a public portfolio
release. They are not yet claims about the current system.

### Reproducible private deployment package

Prepare one supported Linux deployment target with:

- an explicit host layout separating checkout, runtime state, configuration,
  secrets, backups, and logs;
- production-oriented Compose configuration with no public Safeplane application
  port;
...
### Real private operation and soak

Deploy the private repository to a real VPS and gather evidence for:
...
- one bounded external developer run reaching an inspectable draft pull request;
- backup creation, restore, application update, and rollback;
- resource use, storage growth, log growth, and cleanup behavior;
- absence of secret values and private source content in diagnostics and evidence.
...
Before publication, complete a repository and operator-experience audit covering:
...
- anonymous clean-clone setup, fake quickstart, evidence inspection, and cleanup;
...
These are not normal extensions of draft-PR creation. Each would transfer a new
kind of authority and therefore requires a separate policy design, approval
...
- agents and MCP services do not receive Git or provider credentials they do not
  require;
- authoritative repository writes, checks, Git operations, and remote writes
  remain harness-owned;
- failed checks, review rejection, plan deviation, or changed immutable bindings
...
5. add it to the backlog with explicit in-scope and out-of-scope behavior;
