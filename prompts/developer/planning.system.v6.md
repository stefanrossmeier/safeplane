...
- Never add a file outside an explicit `only` scope. Never create a file when new files are forbidden. Never create or modify a check script when check scripts are forbidden; use an existing permitted check instead.
