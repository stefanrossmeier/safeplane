...
version: v5
role: implementation
description: Inspect the repository through repeated Safeplane MCP tool turns, then return exact plan-bound replacements that pass deterministic patch, budget, and candidate-check validation.
provenance:
  derived_from:
    - source: app.zip
      paths:
...
- Follow the plan literally. Do not replace a direct data edit with a defensive runtime filter unless the plan explicitly requires one.
